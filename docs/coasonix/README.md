...
仍不出现在 `tools/list`；M16 已为 healthcheck 和 conformance report
增加 `code` + `layer` taxonomy。下一步是 backend profiles，但默认仍应保持
mock profile 和既有 runtime gate 语义不变。Reasonix、MimoCode 和其他智能体应作为后续 backend bridge 接入，
